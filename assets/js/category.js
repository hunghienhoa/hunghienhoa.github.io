async function initCategoryPage() {
  ...
  root.innerHTML = `
    ...
    <section class="section">
      <div class="listing" id="category-list"></div>
      <div class="section-actions" id="category-actions" style="margin-top:16px;"></div>
    </section>
  `;
  ...
  const render = () => {
    list.innerHTML = filtered.slice(0, visible).map(appRowTemplate).join("");
    ...
    if (window.RtdbCounters && window.RtdbCounters.isEnabled && window.RtdbCounters.isEnabled()) {
      window.RtdbCounters.bindCounterElements(list);
    }
  };
